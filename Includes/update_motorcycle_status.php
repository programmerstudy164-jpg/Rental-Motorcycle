...
if (isset($_GET['action']) && isset($_GET['id_moto'])) {
    $action = $_GET['action'];
    ...
    $query = "UPDATE motorcycle SET status = '$new_status' WHERE id_moto = $id_moto";

    if (mysqli_query($conn, $query)) {
        echo "<script>alert('Motorcycle status updated successfully to $new_status'); window.location.href='../status_motorcycle.php';</script>";
    } else {
