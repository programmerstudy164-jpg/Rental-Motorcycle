...
// Bumuo ng base query
$base_query = "SELECT contact.fname, contact.lname, contact.email, user.username, contact.Driver_license, contact.remarks, rentals.status
               FROM contact, rentals, user
               WHERE contact.rental_id = rentals.rental_id
               AND rentals.user_id = user.user_id";

// Magdagdag ng condition kung may status filter
if ($status_filter !== 'All') {
    $base_query .= " AND rentals.status = '" . mysqli_real_escape_string($conn, $status_filter) . "'";
}
...
$total_query = "SELECT COUNT(*) AS total 
                ...
                AND rentals.user_id = user.user_id";
if ($status_filter !== 'All') {
    $total_query .= " AND rentals.status = '" . mysqli_real_escape_string($conn, $status_filter) . "'";
}
...
$base_query .= " ORDER BY FIELD(rentals.status, 'Pending', 'Approved', 'Declined', 'Cancelled') ASC LIMIT $start, $limit";
    ...
    <div class="table-container">
        <div class="table-title" style="background: linear-gradient(130deg, rgba(34, 193, 195, 1) 0%, rgba(253, 187, 45, 1) 100%); border-radius: 8px 8px 0 0;">
            <div class="d-flex justify-content-between align-items-center">
                <h2 class="mb-0 text-white"><i class="fas fa-calendar-check"></i> Booking Information</h2>
