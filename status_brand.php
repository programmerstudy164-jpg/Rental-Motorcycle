...
<li class="menu-item">
  <a  class="active" class="menu-toggle" data-toggle="collapse" href="#status" role="button" aria-expanded="false">
...
// Bumuo ng base query para sa counting
$count_query = "SELECT COUNT(*) as total FROM brand";
$conditions = [];
...
// Kunin ang total entries
$total_result = mysqli_query($conn, $count_query);
$total_row = mysqli_fetch_assoc($total_result);
$total_entries = $total_row['total'];
$total_pages = ceil($total_entries / $limit);

// Query para kunin ang brands with filter, pagination, at join sa user
$brand_query = "
  SELECT brand.*, user.role 
  FROM brand 
  LEFT JOIN user ON brand.added_by = user.user_id
";
if (count($conditions) > 0) {
    $brand_query .= " WHERE " . implode(" AND ", $conditions);
}
$brand_query .= " ORDER BY FIELD(status, 'Pending', 'Approved', 'Declined', 'Cancelled') ASC LIMIT $start, $limit";
  ...
  <div class="table-container">
    <div class="table-title" style="background: linear-gradient(130deg, rgba(34, 193, 195, 1) 0%, rgba(253, 187, 45, 1) 100%); border-radius: 8px 8px 0 0;">
      <div class="d-flex justify-content-between align-items-center">
        <h2 class="mb-0 text-white"><i class="fas fa-calendar-check"></i> Status Brand</h2>
          ...
          <tr>
            ...
            <td>
              ...
              <div class="booking-actions">
                <a href="Includes/update_brand_status.php?action=approve&brand_id=<?= $row['brand_id'] ?>" class="approve" onclick="return confirm('Approve this brand?');" title="Approve Brand">
                  <i class="fas fa-clipboard-check text-success" style="font-size: 20px;"></i>
                </a>
                <a href="Includes/update_brand_status.php?action=decline&brand_id=<?= $row['brand_id'] ?>" class="decline" onclick="return confirm('Decline this brand?');" title="Decline Brand">
        ...
        <nav>
          <ul class="pagination pagination-sm">
            <li class="page-item <?php if ($page <= 1) echo 'disabled'; ?>">
              <a class="page-link" href="?status=<?php echo $status_filter; ?>&page=<?php echo $page - 1; ?>">&laquo;</a>
            </li>
            ...
            if ($start_page > 1): ?>
              <li class="page-item"><a class="page-link" href="?status=<?php echo $status_filter; ?>&page=1">1</a></li>
              <?php if ($start_page > 2): ?>
            ...
            <?php if ($end_page < $total_pages): ?>
              <?php if ($end_page < $total_pages - 1): ?>
                <li class="page-item disabled"><span class="page-link">...</span></li>
              <?php endif; ?>
              <li class="page-item"><a class="page-link" href="?status=<?php echo $status_filter; ?>&page=<?php echo $total_pages; ?>"><?php echo $total_pages; ?></a></li>
            <?php endif; ?>

            <li class="page-item <?php if ($page >= $total_pages) echo 'disabled'; ?>">
              <a class="page-link" href="?status=<?php echo $status_filter; ?>&page=<?php echo $page + 1; ?>">&raquo;</a>
            </li>
          </ul>
        </nav>
      </div>
