...
// Bumuo ng base query para sa counting ng total entries
$count_query = "
    SELECT COUNT(*) as total
    FROM motorcycle
    LEFT JOIN user ON motorcycle.user_id = user.user_id
    LEFT JOIN brand ON motorcycle.brand_id = brand.brand_id
";

$conditions = [];

if ($status_filter !== 'All') {
    $conditions[] = "motorcycle.status = '" . mysqli_real_escape_string($conn, $status_filter) . "'";
}
...
// Query with actual data + pagination
$data_query = "
    SELECT motorcycle.*, 
           user.username,
           brand.Vehicle_brand AS brand_name
    FROM motorcycle
    LEFT JOIN user ON motorcycle.user_id = user.user_id
    LEFT JOIN brand ON motorcycle.brand_id = brand.brand_id
";
...
if (!empty($conditions)) {
    $data_query .= " WHERE " . implode(" AND ", $conditions);
}

$data_query .= " ORDER BY FIELD(motorcycle.status, 'Pending', 'Approved', 'Declined', 'Cancelled') ASC LIMIT $start, $limit";
  ...
  <div class="table-container">
    <div class="table-title" style="background: linear-gradient(130deg, rgba(34, 193, 195, 1) 0%, rgba(253, 187, 45, 1) 100%); border-radius: 8px 8px 0 0;">
      <div class="d-flex justify-content-between align-items-center">
        <h2 class="mb-0 text-white"><i class="fas fa-calendar-check"></i> Status Motorcycle</h2>
        <div class="btn-group" role="group">
          <?php
            $statuses = ['All', 'Pending', 'Approved', 'Declined'];
            foreach ($statuses as $status) {
              $active = ($status_filter === $status) ? 'active' : '';
              echo "<a href='?status=$status' class='btn btn-light $active'>$status</a>";
            }
          ?>
        </div>
      </div>
        ...
        <tbody>
          <?php while ($row = mysqli_fetch_assoc($result)): ?>
            <tr>
              ...
              <td><img src="<?php echo htmlspecialchars($row['image']); ?>" width="50" alt="Vehicle Image"></td>
              <td><?php echo htmlspecialchars($row['status']); ?></td>
              <td>
                <?php if ($row['status'] === 'Pending'): ?>
                  <div class="booking-actions">
                    <a href="Includes/update_motorcycle_status.php?action=approve&id_moto=<?= $row['id_moto'] ?>" onclick="return confirm('Approve this motorcycle?');" title="Approve">
                      <i class="fas fa-clipboard-check text-success" style="font-size: 20px;"></i>
                    </a>
                    <a href="Includes/update_motorcycle_status.php?action=decline&id_moto=<?= $row['id_moto'] ?>" onclick="return confirm('Decline this motorcycle?');" title="Decline">
        ...
        <nav>
          <ul class="pagination pagination-sm">
            ...
            for ($i = $start_page; $i <= $end_page; $i++) {
              ...
              echo "<li class='page-item $active'><a class='page-link' href='?status=$status_filter&page=$i'>$i</a></li>";
            }
