...
if ($role === 'admin') {
  ...
  header("Location: login.php"); // Redirect if role is undefined or not authorized
  exit();
}
...
<?php } ?>
      ...
      <li class="menu-item">
        ...
        <li  class="menu-item">
          <a class="active" class="menu-toggle " data-toggle="collapse" href="#vehiclesMenus1" role="button" aria-expanded="false">
  ...
  <div class="table-container">
    <div class="table-title" style="background: linear-gradient(130deg, rgba(34, 193, 195, 1) 0%, rgba(253, 187, 45, 1) 100%); border-radius: 8px 8px 0 0;">
      <div class="d-flex justify-content-between align-items-center">
        <h2 class="mb-0 text-white"><i class="fas fa-users me-2"></i>Manage Admin</h2>
...
// Modify the WHERE clause to show only staff
$where = "WHERE role = 'admin'"; // Use 'role' instead of 'usertype'
...
$total_query = "SELECT COUNT(*) AS total FROM user $where";
...
// Paginated result
$query = "SELECT * FROM user $where LIMIT $start, $limit";
      ...
      <?php if ($result->num_rows > 0): ?>
        <?php while ($row = $result->fetch_assoc()): ?>
          <tr>
            ...
            <?php
  $logged_in_user_id = $_SESSION['user_id']; // kasalukuyang naka-login na admin
  $is_super_admin = $row['user_id'] == 1;
  $is_logged_admin = $row['user_id'] == $logged_in_user_id;
?>

<?php if ($is_super_admin): ?>
  <span class="badge bg-secondary px-2 py-1">Protected</span>

<?php elseif ($is_logged_admin): ?>
  <span class="badge bg-info px-2 py-1">Online</span>

<?php else: ?>
  <a href="#myModal" data-id="<?= $row['user_id']; ?>" class="delete text-danger d-inline-flex align-items-center" data-toggle="modal">
    <i class="material-icons mr-1" data-toggle="tooltip" title="Delete">&#xE872;</i> Delete
  </a>

  <a href="edit_admin.php?user_id=<?= $row['user_id']; ?>" class="text-primary d-inline-flex align-items-center" data-toggle="tooltip" title="Edit">
    <i class="material-icons mr-1">&#xE254;</i> Edit
  </a>
<?php endif; ?>
        ...
        <tr>
          <td colspan="11" class="text-center py-4">
            <div class="d-flex flex-column align-items-center">
              ...
              <p class="text-muted">Try changing your keyword or add users.</p>
            </div>
          </td>
        </tr>
      <?php endif; ?>
  ...
  <div class="modal-dialog modal-confirm">
    <div class="modal-content">
      ...
      <div class="modal-footer justify-content-center">
        <form method="POST" action="Includes/delete_admin.php">
          <input type="hidden" name="user_id" id="delete-id"> <!-- Hidden input for user_id -->
...
//delete
$(document).ready(function() {
    // Debug: Make sure the script is loaded
    console.log('JavaScript Loaded');

    // When the delete link is clicked
    $('.delete').on('click', function() {
        // Retrieve the user_id from the data-id attribute of the clicked element
        var userId = $(this).data('id');
        
        // Debugging: Log the userId to make sure it's passed correctly
        console.log('Debug: User ID =', userId); // This should appear in the console
