...
<?php } ?>
      <li class="menu-item">
        ...
        <li class="menu-item">
          <a  class="active" class="menu-toggle " data-toggle="collapse" href="#vehiclesMenus" role="button" aria-expanded="false">
...
// Tukuyin ang kasalukuyang page
$page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
$limit = 10;
$start = ($page - 1) * $limit;

// Tukuyin ang status filter kung mayroon
...
// Magdagdag ng condition kung may status filter
if ($status_filter !== 'All') {
    $conditions[] = "status = '" . mysqli_real_escape_string($conn, $status_filter) . "'";
}

// Idagdag ang mga condition sa query kung mayroon
if (count($conditions) > 0) {
    $base_query .= " WHERE " . implode(" AND ", $conditions);
}

// Kunin ang kabuuang bilang ng entries para sa pagination
$total_query = str_replace("SELECT *", "SELECT COUNT(*) as total", $base_query);
$total_result = mysqli_query($conn, $total_query);
$total_row = mysqli_fetch_assoc($total_result);
$total_entries = $total_row['total'];
$total_pages = ceil($total_entries / $limit);

// Idagdag ang order at limit sa query
$base_query .= " ORDER BY FIELD(status, 'Pending', 'Approved', 'Declined', 'Cancelled') ASC LIMIT $start, $limit";
  ...
  <div class="table-container">
    <div class="table-title" style="background: linear-gradient(130deg, rgba(34, 193, 195, 1) 0%, rgba(253, 187, 45, 1) 100%); border-radius: 8px 8px 0 0;">
      <div class="d-flex justify-content-between align-items-center">
        <h2 class="mb-0 text-white"><i class="fas fa-calendar-check"></i> Manage Booking</h2>
        <div class="btn-group" role="group" aria-label="Status Filter">
          <a href="?status=All" class="btn btn-light <?php if($status_filter=='All') echo 'active'; ?>">All</a>
          <a href="?status=Pending" class="btn btn-light <?php if($status_filter=='Pending') echo 'active'; ?>">Pending</a>
          <a href="?status=Approved" class="btn btn-light <?php if($status_filter=='Approved') echo 'active'; ?>">Approved</a>
          <a href="?status=Declined" class="btn btn-light <?php if($status_filter=='Declined') echo 'active'; ?>">Declined</a>
          <a href="?status=Cancelled" class="btn btn-light <?php if($status_filter=='Cancelled') echo 'active'; ?>">Cancelled</a>
        </div>
      </div>
    </div>
    <div class="table-responsive" style="overflow-x: auto;">
  <table class="table table-striped table-hover align-middle" id="motorcycleTable" style="min-width: 1000px;">
    <thead class="table-light">
      <tr>
        <th style="white-space: nowrap; font-size: 12px">#</th>
        <th style="white-space: nowrap; font-size: 12px">Username</th>
        <th style="white-space: nowrap;font-size: 12px">Brand</th>
        <th style="white-space: nowrap;font-size: 12px">Pickup Location</th>
        <th style="white-space: nowrap;font-size: 12px">Pickup Custom Location</th>
        <th style="white-space: nowrap;font-size: 12px">Return Custom Location</th>
        <th style="white-space: nowrap;font-size: 12px">Return Location</th>
        <th style="white-space: nowrap;font-size: 12px">Pickup Date & Time</th>
        <th style="white-space: nowrap;font-size: 12px">Return Date & Time</th>
        <th style="white-space: nowrap;font-size: 12px">Total Cost</th>
        <th style="white-space: nowrap;font-size: 12px">Status</th>
        <th style="white-space: nowrap;font-size: 12px">Action</th>
      </tr>
    </thead>
    <tbody>
  <?php
  $grandTotal = 0; // Add this to track the total cost
  while ($row = mysqli_fetch_assoc($result)) {
      $id_moto = $row['id_moto'];
      $user_id = $row['user_id'];

      // Kunin ang brand
      $brand_query = "SELECT Vehicle FROM motorcycle WHERE id_moto = $id_moto";
      $brand_result = mysqli_query($conn, $brand_query);
      $brand_row = mysqli_fetch_assoc($brand_result);
      $brand = $brand_row['Vehicle'] ?? 'Unknown';

      // Kunin ang username
      $username_query = "SELECT username FROM user WHERE user_id = $user_id";
      $username_result = mysqli_query($conn, $username_query);
      $username_row = mysqli_fetch_assoc($username_result);
      $username = $username_row['username'] ?? 'Unknown';

      // Kunin ang total cost mula summary
      $rental_id = $row['rental_id'];
      $summary_query = "SELECT total_cost FROM summary WHERE rental_id = $rental_id";
      $summary_result = mysqli_query($conn, $summary_query);
      $summary_row = mysqli_fetch_assoc($summary_result);
      $total_cost = $summary_row['total_cost'] ?? 0;
      $grandTotal += $total_cost;

      // Kalkulahin ang duration
      $duration = 'N/A';
      if (!empty($row['pickup_datetime']) && !empty($row['return_datetime'])) {
          $pickup = new DateTime($row['pickup_datetime']);
          $return = new DateTime($row['return_datetime']);
          $interval = $pickup->diff($return);
          $duration = $interval->format('%d days %h hrs');
      }
  ?>
  <tr>
    ...
    <td><?php echo htmlspecialchars($row['custom_pickup_location']); ?></td>
    <td><?php echo htmlspecialchars($row['custom_return_location']); ?></td>
    <td><?php echo htmlspecialchars($row['return_location']); ?></td>
    <td><?php echo htmlspecialchars($row['pickup_datetime'] ?? 'N/A'); ?></td>
    <td><?php echo htmlspecialchars($row['return_datetime'] ?? 'N/A'); ?></td>
    
    <td>₱<?php echo number_format($total_cost, 2); ?></td> <!-- total cost shown here -->
    <td><?php echo htmlspecialchars($row['status']); ?></td>
    <td>
      <?php if ($row['status'] === 'Pending') : ?>
      <div class="booking-actions">
        <a href="Includes/update_status.php?action=approve&rental_id=<?= $row['rental_id'] ?>" class="approve" onclick="return confirm('Approve this booking?');" title="Approve Booking">
          <i class="fas fa-clipboard-check text-success" style="font-size: 20px;"></i>
        </a>
        <a href="Includes/update_status.php?action=decline&rental_id=<?= $row['rental_id'] ?>" class="decline" onclick="return confirm('Decline this booking?');" title="Decline Booking">
      ...
      <div class="d-flex justify-content-between align-items-center">
        ...
        <nav>
          <ul class="pagination pagination-sm">
            ...
            if ($start_page > 1): ?>
              <li class="page-item">
                <a class="page-link" href="?status=<?php echo $status_filter; ?>&page=1">1</a>
              </li>
              <?php if ($start_page > 2): ?>
            ...
            <?php if ($end_page < $total_pages): ?>
              ...
              <?php endif; ?>
              <li class="page-item">
                <a class="page-link" href="?status=<?php echo $status_filter; ?>&page=<?php echo $total_pages; ?>"><?php echo $total_pages; ?></a>
              </li>
            <?php endif; ?>
