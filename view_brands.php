...
<?php } ?>
      ...
      <li class="menu-item">
          <a class="active" class="menu-toggle" data-toggle="collapse" href="#brandsMenu" role="button" aria-expanded="false">
  ...
  <div class="table-container">
    <div class="table-title" style="background: linear-gradient(130deg, rgba(34, 193, 195, 1) 0%, rgba(253, 187, 45, 1) 100%); border-radius: 8px 8px 0 0;">
      <div class="d-flex justify-content-between align-items-center">
        <h2 class="mb-0 text-white"><i class="fas fa-users me-2"></i>Manage Brands</h2>
       <?php if($_SESSION['role'] != "admin"){ ?>
    ...
    <table class="table table-striped table-hover align-middle" id="motorcycleTable">
      ...
      <tbody>
        ...
        if ($status_filter != 'All') {
          $status_condition = " AND brand.status = '$status_filter'";
        }
        
        $keyword = $_GET['keyword'] ?? '';
        $keyword = $conn->real_escape_string($keyword);

        // Base SQL query to fetch brands
        $sql = "SELECT brand.*, user.role
                FROM brand
                INNER JOIN user ON brand.added_by = user.user_id
                WHERE 1=1 $status_condition";

        // Apply keyword search filter
        if (!empty($keyword)) {
          $sql .= " AND (Vehicle_brand LIKE '%$keyword%' OR creation_date LIKE '%$keyword%' OR updation_date LIKE '%$keyword%')";
        }

        // If the user is an Admin, show only approved brands
        if ($_SESSION['role'] == 'admin') {
          $sql .= " AND brand.status = 'approved'";
        }

        // Add sorting by status (ascending or descending based on user selection)
        $order_by = isset($_GET['sort']) && $_GET['sort'] == 'desc' ? 'DESC' : 'ASC';
        $sql .= " ORDER BY brand.status $order_by";
        ...
        // Total entries query (including keyword filter)
        $total_query = "SELECT COUNT(*) AS total
                        FROM brand
                        INNER JOIN user ON brand.added_by = user.user_id
                        WHERE 1=1 $status_condition";

        // Apply keyword search filter for total entries
        if (!empty($keyword)) {
          $total_query .= " AND (Vehicle_brand LIKE '%$keyword%' OR creation_date LIKE '%$keyword%' OR updation_date LIKE '%$keyword%')";
        }

        // Same filtering rule for total count
        if ($_SESSION['role'] == 'admin') {
          $total_query .= " AND brand.status = 'approved'";
        }
        ...
        <?php if ($result->num_rows > 0): ?>
          <?php while ($row = $result->fetch_assoc()): ?>
            <tr>
              ...
              <?php if ($_SESSION['role'] != 'admin') { ?>
                <!-- Staff or other non-admin roles: display status -->
                <td><?= $row['status'] ?></td>
              <?php } ?>

              <?php if ($_SESSION['role'] == 'admin') { ?>
                <!-- Admin: Can see edit and delete options -->
                <td>
                  <a href="edit_brand.php?id=<?= $row['brand_id'] ?>" class="edit">
                    <i class="material-icons" data-toggle="tooltip" title="Edit">&#xE254;</i>
                  </a>
                  <a href="#myModal" data-id="<?= $row['brand_id']; ?>" class="delete" data-toggle="modal">
  ...
  <div class="modal-dialog modal-confirm">
    <div class="modal-content">
      ...
      <div class="modal-footer justify-content-center">
        <form method="POST" action="Includes/delete_data_brand.php">
          <input type="hidden" name="brand_id" id="delete-id">
