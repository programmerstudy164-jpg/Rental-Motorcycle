...
// Get the selected status filter, default to 'All' if not set
...
// Main query to get the total count of motorcycles with filter for status
$where = "";
if (!empty($keyword)) {
    $where = "WHERE Vehicle LIKE '%$keyword%' OR Transmission LIKE '%$keyword%' OR Model_Year LIKE '%$keyword%' OR Displacement LIKE '%$keyword%'";
}

// Apply status filter to query
if ($status_filter != 'All') {
    $where .= " AND motorcycle.status = '$status_filter'";
}

// Add condition based on role (Staff sees all, Admin sees only approved)
if ($_SESSION['role'] == 'admin') {
    $where .= " AND motorcycle.status = 'approved'";
}

// Total entries (with filter)
$total_query = "SELECT COUNT(*) AS total FROM motorcycle INNER JOIN user ON motorcycle.user_id = user.user_id $where";
...
// Main query (with filter + limit + sorting)
$query = "SELECT motorcycle.*, user.username, user.role FROM motorcycle 
          INNER JOIN user ON motorcycle.user_id = user.user_id 
          $where 
          ORDER BY motorcycle.status $order_by 
          LIMIT $start, $limit";
  ...
  <div class="table-container">
    <div class="table-title" style="background: linear-gradient(130deg, rgba(34, 193, 195, 1) 0%, rgba(253, 187, 45, 1) 100%); border-radius: 8px 8px 0 0;">
      <div class="d-flex justify-content-between align-items-center">
        <h2 class="mb-0 text-white"><i class="fas fa-users me-2"></i>Manage Vehicles</h2>
        <?php if($_SESSION['role'] != "admin"){ ?>
        ...
        <tbody>
          ...
          ?>
            <tr>
              ...
              <td><?php echo htmlspecialchars($brand['Vehicle_brand']); ?></td>
              ...
              <td>
                <img src="<?php echo htmlspecialchars($row['image']); ?>" alt="Vehicle Image" style="width: 80px; height: auto;">
              </td>
              ...
              <?php } elseif ($_SESSION['role'] == 'admin') { ?>
                <td>
                  <a href="edit_vehicle.php?id=<?= $row['id_moto'] ?>" class="edit">
                    <i class="material-icons" data-toggle="tooltip" title="Edit">&#xE254;</i>
                  </a>
                  <a href="#myModal" data-id="<?= $row['id_moto']; ?>" class="delete" data-toggle="modal">
...
<div id="myModal" class="modal fade">
	<div class="modal-dialog modal-confirm">
		<div class="modal-content">
			...
			<form method="POST" action="Includes/delete_data_vehicle.php">

    <input type="hidden" name="id_moto" id="delete-id">
